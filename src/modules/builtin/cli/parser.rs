...
pub struct ParserImpl {
    params: Vec<Rc<ParamImpl>>,
}

impl ParserImpl {
    pub fn find_parser(meta: &ParserMetadata, parser: &Expr) -> Option<Rc<RefCell<ParserImpl>>> {
        if let Some(name) = parser.get_var_translated_name() {
            if let Some(var) = meta.get_var(&name) {
    ...
    pub fn add_param(&mut self, param: Rc<ParamImpl>) {
...
impl SyntaxModule<ParserMetadata> for ParserCli {
    ...
    fn parse(&mut self, meta: &mut ParserMetadata) -> SyntaxResult {
        ...
        token(meta, "(")?;
        token(meta, ")")?;
        let parser = ParserImpl { params: Vec::new() };
